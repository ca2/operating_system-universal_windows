...
namespace windows
{
   ...
   bool printer::open(const scoped_string & strDeviceName)
   {
      ...
      if(!OpenPrinter((char *) (const_char_pointer )pszDeviceName, &m_hPrinter, nullptr))
         return false;
   ...
   bool printer::document_properties::initialize(::windows::printer * pprinter, DEVMODE * pdevmode)
   {
      ...
      if(m_hdc != nullptr)
         return false;
      int iSize = DocumentProperties(nullptr, pprinter->m_hPrinter, (char *)(const_char_pointer )pprinter->m_strName, nullptr, nullptr, 0);
      m_pdevmode = (DEVMODE *) malloc(iSize);
      if(!DocumentProperties(nullptr, pprinter->m_hPrinter, (char *) (const_char_pointer )pprinter->m_strName, m_pdevmode, nullptr, DM_OUT_BUFFER))
      {
   ...
   ::draw2d::graphics * printer::document_properties::create_graphics()
   {
      ...
      if(m_hdc != nullptr)
         return nullptr;
      m_hdc = ::CreateDC("WINSPOOL", (const_char_pointer )m_pdevmode->dmDeviceName, nullptr, m_pdevmode);
