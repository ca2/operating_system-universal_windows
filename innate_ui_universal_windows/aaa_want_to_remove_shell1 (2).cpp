...
namespace windows
{
   ...
   /*   BOOL shell::_SHGetPathFromIDList(LPCITEMIDLIST pidl, unichar * pszPath)
      ...
      HANDLE shell::_FindFirstFile(const unichar * pcsz, UNIVERSAL_WINDOWS_FIND_DATAW * pdata)

      {
         string
         unicode_to_utf8(pszPathA, MAX_PATH * 2, pcsz);

         UNIVERSAL_WINDOWS_FIND_DATAA data;
      ...
      BOOL shell::_FindNextFile(HANDLE handle, UNIVERSAL_WINDOWS_FIND_DATAW * pdata)
