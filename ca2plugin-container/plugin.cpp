...
/*   UINT __cdecl plugin::ThreadProcStart(LPVOID lpv)
   ...
   void plugin::run_start_install(const ::scoped_string & scopedstrInstall)
   {
   
      {
         ...
         file_system()->put_contents(dir::appdata("spa_install.xml"), node.GetXML(NULL));
      }
install:
...
/*      win::thread * pthread = dynamic_cast < win::thread *> (papp->smart_pointer < ::ca::thread > ::m_p);
  ...
  /*    ::SetEvent(m_hEventReady);
   ...
   //   pplugin->m_pframe = dynamic_cast < bergedge::frame * > (pplugin->m_papp->get_edge(0, true)->get_bergedge_impact()->get_parent_frame());
   ...
   /*   papp->m_puiInitialPlaceHolderContainer = pplugin->m_pframe;
      ...
      try
      {
         ...
         //informationf("Exit Code from retract_app %d (exit_instance %s)", iExitCode, typeid(*papp).raw_name());
      }
      catch(...)
      {
      }
      ...
      default:
         {
            ...
            if((uiMessage == ::user::e_message_lbutton_up
            || uiMessage == ::user::e_message_rbutton_up
