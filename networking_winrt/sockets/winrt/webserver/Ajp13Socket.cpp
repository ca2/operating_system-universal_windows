...
namespace sockets
{
   ...
   void Ajp13Socket::OnHeader( short atom, short len )
   {
      ...
      {
         informationf("Packet int_size: %d bytes\n", len);
      }
   }
   ...
   void Ajp13Socket::ReceiveForwardRequest( const_char_pointer buf, memsize sz )
   {
      ...
      psession->sockets().m_pajpbasesocketinit->Method.find(method, method_str);
         ...
         case 0xa0:
            {
               ...
               if (!psession->sockets().m_pajpbasesocketinit->header.find(x, key))
               {
      ...
      // int_size left to read from web server
      ...
      while ( (unsigned char)buf[ptr] != 0xff)
      {
         ...
         default:
            {
               if(!psession->sockets().m_pajpbasesocketinit->Attribute.find(code, key))
               {
   ...
   //   0x9 CPong Reply
      ...
      // Send Headers
      {
         ...
         for (auto & prop : m_response.headers())
         {
            ...
            if(psession->sockets().m_pajpbasesocketinit->ResponseHeader.find(strNameLower, iValue))
            {
   ...
   void Ajp13Socket::OnPacket( const_char_pointer buf, memsize sz )
   {
      ...
      // check body int_size left to read, if non-zero packet is body data
