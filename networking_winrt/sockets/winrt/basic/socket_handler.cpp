         ...
         if(psocket != nullptr)
         {
            ...
            if(pstreamsocket != nullptr)
            {
               if(pstreamsocket->m_writer != nullptr)
               {
                  ...
                  writer->StoreAsync()->Completed = ref allocateø< ::winrt::Windows::Foundation::AsyncOperationCompletedHandler < unsigned int >  >([=]
...
//#if defined(BSD_STYLE_SOCKETS)
   ...
   {
      // check cache
      resolv_socket *resolv = ___new resolv_socket(*this, p, a);
      resolv -> SetId(++m_resolv_id);
      resolv -> SetDeleteByHandler();
//      ipaddr_t local;
      ::networking::address addressLocal("127.0.0.1",m_resolver_port);
   ...
   void socket_handler::EnableResolver(port_t port)
   {
      if (!m_resolver)
      {
         ...
         m_resolver = allocateø< resolv_server(get_app >(), port);
      }
   }
